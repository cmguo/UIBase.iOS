...
public class XHBTextView : UITextView {
    ...
    private var _style: XHBTextViewStyle
    ...
    private let _delegate = XHBTextViewDelegate()
    private let _placeholderAttrs: [NSAttributedString.Key:Any]

    
    public init(style: XHBTextViewStyle = .init()) {
        _style = style
        _placeholderAttrs = [
...
class XHBTextViewDelegate : UITextViewDelegateWrapper {
    ...
    override func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText string: String) -> Bool { // return NO to not change text
        ...
        if let tf = textView as? XHBTextView, let text = tf.text,
    ...
    override func textViewDidChange(_ textView: UITextView) {
        ...
        if let tf = textView as? XHBTextView {
            tf.setNeedsDisplay()
        }
    }
}

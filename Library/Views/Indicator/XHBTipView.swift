...
public class XHBTipView : UIView
{
    ...
    private let _style: XHBTipViewStyle
    private let _backLayer = CALayer()
    private let _arrowLayer = CAShapeLayer()

    public init(_ style: XHBTipViewStyle = .init()) {
        self._style = style
    ...
    private func syncAppearance() {
        let a: XHBTipViewAppearance = tipAppearance ?? (location < Location.AutoToast ? .ToolTip : (location == Location.AutoToast ? .Toast : .Snack))
    ...
    @objc func buttonClicked(_ sender: UIView) {
        callback?.tipViewButtonClicked?(self, (sender as! XHBButton).id)
    }
    ...
    private class OverlayFrame : UIView, UIGestureRecognizerDelegate {
        ...
        private var tipList: [XHBTipView] = []
        ...
        fileprivate func attach(_ tip: XHBTipView) {
        ...
        fileprivate func detach(_ tip: XHBTipView) {

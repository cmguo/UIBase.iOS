...
public enum SwiftPopTipAnimation: Int {
    case slide = 0
    case pop
}

public protocol SwiftPopTipViewDelegate {
    func popTipViewWasDismissedByUser(_ popTipView: SwiftPopTipView)
}

public class SwiftPopTipView: UIView {
    
    public static func tip(view: UIView,
                    text: String,
                    direction: PointDirection = .down,
                    textColor: UIColor? = ThemeColor.shared.static_white_100,
                    popColor: UIColor? = UIColor(rgb: 0x111111),
                    maxWidth: CGFloat = -50,
                    containerView: UIView? = nil,
                    autoDismiss: Bool? = true,
                    delayTime: Double = 2,
                    dismissTapAnywhere: Bool = true) {
        let tipView = SwiftPopTipView(title: nil, message: text, customView: nil)
        tipView.preferredPointDirection = direction
        tipView.sidePadding = 10
        tipView.textColor = textColor ?? UIColor.clear
        tipView.popColor = popColor ?? UIColor.clear
        tipView.maxWidth = maxWidth < 0 ? view.window!.bounds.width + maxWidth : maxWidth
        tipView.presentPointingAtView(view, containerView: containerView ?? view.superview!, animated: true)
        tipView.dismissTapAnywhere = dismissTapAnywhere

        if let autoDismiss = autoDismiss, autoDismiss {
    ...
    public var dismissBlock: ((SwiftPopTipView) -> Void)?
    public var delegate: SwiftPopTipViewDelegate?
    public var disableTapToDismiss = false
    public var dismissTapAnywhere = true
    public var isShowing = false
    public var title: String?
    public var message: String?
    public var customView: UIView?
    public var targetObject: Any?
    public var popColor = UIColor.white
    public var titleColor: UIColor?
    public var titleFont = UIFont.boldSystemFont(ofSize: 16)
    public var textColor = UIColor.black
    public var textFont = UIFont.boldSystemFont(ofSize: 14)
    public var titleAlignment: NSTextAlignment = .center
    public var textAlignment: NSTextAlignment = .center
    public var has3DStyle = false
    ...
    public var animation: SwiftPopTipAnimation = .slide
    public var maxWidth: CGFloat?
    public var preferredPointDirection: PointDirection = .any
    private var pointDirection: PointDirection = .any
    public var hasGradientBackground = false
    public var sidePadding: CGFloat = 2
    public var topMargin: CGFloat = 2
    public var pointerSize: CGFloat = 12
    public var bubbleSize: CGSize = CGSize.zero
    public var targetPoint: CGPoint = CGPoint.zero
    public var autoDismissTimer: Timer?
    public var dismissTarget: UIButton?
    ...
    public init(title: String? = nil, message: String? = nil, customView: UIView? = nil) {
        super.init(frame: CGRect.zero)
        self.title = title
    ...
    public func presentPointingAtBarButtonItem(_ barButtonItem: UIBarButtonItem, animated: Bool) {
        ...
        presentPointingAtView(targetView, containerView: containerView!, animated: animated)
    }

    public func changePointingAtView(_ targetView: UIView, containerView: UIView) {
        if !isShowing {
            presentPointingAtView(targetView, containerView: containerView, animated: true)
            return
        }
    ...
    public func presentPointingAtView(_ targetView: UIView, containerView: UIView, animated: Bool) {
        if isShowing {
            return
        }
        isShowing = true
        ...
        guard let targetSuperview = targetView.superview, let superView = superview else {
            return
        }
        let targetRelativeOrigin = targetSuperview.convert(targetView.frame.origin, to: superview)
        let containerRelativeOrigin = superView.convert(containerView.frame.origin, to: superview)
    ...
    func autoDismissAnimated(_: Bool, atTimeInterval timeInvertal: TimeInterval) {
    ...
    @objc func dismissTapAnywhereFired(_: UIButton) {
        dismissByUser()
    }
    ...
    @objc func popAnimationDidStop(_: String, finished _: Bool, context _: Any) {
        UIView.beginAnimations(nil, context: nil)
    ...
    public func presentAnimatedPointingAtBarButtonItem(_ barButtonItem: UIBarButtonItem, autodismissAtTime time: TimeInterval) {
        presentPointingAtBarButtonItem(barButtonItem, animated: true)
        autoDismissAnimated(true, atTimeInterval: time)
    }

    public func presentAnimatedPointingAtView(_ atView: UIView, inView: UIView, autodismissAtTime time: TimeInterval) {

...
class XHBListViewCell : UITableViewCell {
    
    private static let DefaultStyle = XHBListItemStyle()

    private lazy var _leftCheckBox: XHBCheckBox = {
        let cb = XHBCheckBox()
        addSubview(cb)
        return cb
    }()
    ...
    var _rightViewType: XHBListItemContentType? = nil
    
    private var _style: XHBListItemStyle = XHBListViewCell.DefaultStyle
    ...
    public func bindData(_ data: XHBListItemProtocol, listView: XHBListView) {
    ...
    override func willMove(toSuperview newSuperview: UIView?) {
        if newSuperview == nil, let r = _rightView {
            _rightView = superview(ofType: XHBListView.self)?.switchRightView(nil, r, _rightViewType)
            _rightViewType = nil
        }
    }

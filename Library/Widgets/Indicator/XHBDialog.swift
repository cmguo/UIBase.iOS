...
public class XHBDialog : UIView {
    ...
    private let _cancelButton = XHBButton().buttonType(.Tertiary).buttonSize(.Middle)
    private let _confirmButton = XHBButton().buttonSize(.Middle)
    private var _moreButtons: [XHBButton] = []
    private var _checkBox = XHBCheckBox()

    private let _style: XHBDialogStyle
    
    public init(style: XHBDialogStyle = .init()) {
        _style = style
    ...
    public func popUp(target: UIView) {
        XHBMaskDialog(content: self, gravity: Gravity.CENTER).show(window: target.window!)
    }
    ...
    @objc private func buttonClicked(_ sender: UIView) {
        callback?.dialogButtonClicked?(dialog: self, btnId: (sender as! XHBButton).id)
    }
    
    @objc private func moreButtonClicked(_ sender: UIView) {
        callback?.dialogMoreButtonClicked?(dialog: self, index: _moreButtons.firstIndex(of: sender as! XHBButton) ?? -1)
    }

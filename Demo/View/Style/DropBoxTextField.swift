...
class DropBoxTextField: UIView, UITableViewDelegate, UITableViewDataSource {
    ...
    private func commonInit(textField: UITextField?) {
        ...
        _button = UIImageView(image: UIImage(named: "drop_down", in: Bundle(for: DropBoxTextField.self), compatibleWith: nil))
        ...
        _tableView.backgroundColor = .gray
    ...
    override func layoutSubviews() {
        ...
        var f = frame
        f.size.height = 44
        f.origin.y = frame.origin.y + frame.size.height / 2 - f.size.height / 2
        frame = f
    }
...
private class SimpleDropBoxCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        // 选中后右边显示一个“勾选”符号
        let selectedView = UIImageView(image: UIImage(named: "check", in: Bundle(for: DropBoxTextField.self), compatibleWith: nil))
